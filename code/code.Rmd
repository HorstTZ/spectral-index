...
png("results/Spectra_corr.png")
ggpubr::ggarrange(s1, p1, p4,  ncol=1, nrow=3)
dev.off()

png("results/Spectra_corr_CRR.png")
...
png("results/Spectra_corr_SGD.svg")
ggpubr::ggarrange(s1, p1, p4, s2, p2, p5, s3, p3, p6,  ncol=1, nrow=9)
dev.off()
...
```{r}
#dat <- as.data.frame(dataindex)
#write.csv(dat, "SLL_index.csv")

dat <- read.csv("SLL_index.csv", sep = ";", header = TRUE, stringsAsFactors=FALSE)
dat <- dat[-c(1), ]
...
# Remover covariaveis com na
    ...
    dat <- dat %>% dplyr::select(-one_of(nearZeroVar(dat, names = T)))  
...
fm <-  as.formula(paste("soc ~", paste(names(dat)[c(seq(4,length(dat)))], collapse = "+")))
...
training <- data.frame(rmse=numeric(), r2=numeric(), bias = numeric(), mae = numeric(), mdae = numeric(), mtry = numeric())
validation <- data.frame(rmse=numeric(), r2=numeric(), bias = numeric(), mae = numeric(), mdae = numeric())
#importance <- data.frame(names(spectra))
